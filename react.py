...
async def root():
    ...
    return {
        "status": "online",
        "service": "Telegram VC Bot API (py-tgcalls 0.9.7)",
        "endpoints": [
            "/health",
...
@app.get("/volume")
async def set_volume(chat_id: int, volume: int = 200):
    """Set volume (100 = 100%, 200 = 200%, etc.)"""
    global call
    ...
    try:
        # Note: Volume control might not be available in all py-tgcalls versions
        # This is a placeholder implementation
        await call.change_volume_call(chat_id, volume)
        return {"status": "success", "message": f"Volume set to {volume}%"}
    except Exception as e:
        return {"status": "error", "message": f"Volume control not available: {str(e)}"}
